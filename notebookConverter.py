...
def py2nb(pyStr):
  ...
  if pyStr.startswith(header_comment):
    pyStr = pyStr[len(header_comment):]
...
def convert(in_file, out_file):
  ...
  if in_ext == '.ipynb' and out_ext == '.py':
    ...
    pyStr = nb2py(notebook)
    with open(out_file, 'w') as f: f.write(pyStr)

  elif in_ext == '.py' and out_ext == '.ipynb':
    with open(in_file, 'r') as f: pyStr = f.read()
    notebook = py2nb(pyStr)
